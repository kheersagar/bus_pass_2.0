    ...
    try{
      if(!receipt_img) {
        Toast.show({
          type:ALERT_TYPE.DANGER,
          title: 'Error',
          textBody: 'Please Upload Receipt!!',
          button: 'close',
        })
        return
      }
...
export const getBusPass = () =>{
 return async (dispatch) =>{
  try{
    dispatch(userActions.setBusPassLoading(true))
    const res = await API.get('/bus-pass/get-pass');
    dispatch(userActions.setBusPassDetails(res.data))
  }catch(err){
    dispatch(userActions.setBusPassDetails(err.response.data))
      Toast.show({
        type:ALERT_TYPE.DANGER,
        title: 'Error',
...
export const getNotification = () =>{
  return async (dispatch) =>{
    try{
      ...
      dispatch(userActions.setNotificationsLoading(false))
    }
  }
}
